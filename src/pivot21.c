...
/* States of process*/
#define STATE_INIT      0       /* the first state that the prrogram executes in
                                 *the automatic mode*/
...
#define STATE_PROB      6       /* to indicates some issues and stop the
                                 *program*/
...
{
  return /* ppos == true || */ pmotor == true || pwater == true || pelec ==
         true;
}
...
void pivot21_task(void)
{
  ...
  switch (state)
    ...
    case STATE_INIT:
    {
      ...
      /*
       *dio_turnon(4);
       *SYS_TMR_CallbackSingle (3000, 0, stp_timer_cb); to schedule  time
       *operation
       *dio_turnoff(5);
       */
  ...
  #if 0
      if (start_timer_mov == true) /*we can remove the true when the variable is
                                    *bool*/
        {
    ...
    case STATE_MOV:
    {
       ...
       * we need to take in cosideration that angle it moves with, if we don't
       *we would be working with only one stick!!
       */

      if (!(( is_weather_valid() &&
              is_time_valid()) &&
            (( is_eco_time() && eco == true) || eco == false)))
        {
...
app_dbg_msg(" ECO 2\n");
int curr_hr = curr_clock.time.hours;      /* put variable curr_hr equal to the
                                           *actual hour (ask Abdallah) */
app_dbg_msg("time is %d \n", curr_hr );
