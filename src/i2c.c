...
/**
 * I2C is uses a bus topology, where all the slaves shares the same bus. 
 * I2C uses 7bit slave address, the 8th bit sets the mode for read or write.
 * Write mode (8th : 0) or Read mode (8th: 1)
 * The mode bit is added as the last bit in the address, which is the first
 * bit to send and received by the slave device after the start condition.
 * start condition is when SDA is pulled down followed by SCL pulled down by master
 * The start condition is generated automatically by the hardware.
 * Example: if the address is 0x68, we always shift to the left 0x68 << 1 = 0xD0
 * then if we want to write data we leave it as it is (last bit = 0)
 * if we want to Read data we set the bit to 1, 0xD0 | 0x01 = 0xD1
 *
 * Start-condition + address with mode, is always the first thing to send,
 * if the slave device is connected and ready the master must 
 * receice an ACK signal in the 9th bit, or the 9th clock cycle from the begining.
 * of the address, meaning 7bits address + 1 bit mode + ?[slave ACK bit response]
 *
 * Immediatly after receiving the ACK we send in the Regitster  address
 * we want to read from or write to, the devices responds by an ACK on the 
 * 9th clock cycle if the address is valid
 *
 ...
 *   another ACK,then stop with a stop condition (Release SCL then SDA)
 * - if we want to read the register we set we send again a start condition
 *   with read bit (RW)
 ...
 * We continue to read as much as we want, we send an NACK to the device
 * to inform the device to stop sending data or After a write we just stop writing
 * Send a stop bit.
 *
...
void i2c_init(double speed)
{
  ...
  I2C2CONbits.DISSLW    = 1;    /* disable slwe rate control for low speeds like
                                 * 100 kHz */
...
{
  I2C2CONbits.RCEN = 1;     /* receive enable, the bits clears after receiving 8
                             * bits */
