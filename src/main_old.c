...
void SPI_init(void)
{
  /* Initialize the SPI with 50 MHz */
  // As with the uart we set the mapping of the pins

  SDI1R  = 0b0110; // SDI1 to RPB10
  RPC14R = 0b0101; // SDO1 to RPC14
  // The SCK1 is fixed and cannot be mapped

  TRISBbits.TRISB10 = 1; // Set SDI1 to input
  ...
  //
  SPI1CONbits.ON = 0; // Turn off spi before configuring
  SPI1CONbits.MSTEN = 1; //Enable master mode
  SPI1CONbits.CKP = 1;// Clock signal is active low, idle is high
  SPI1CONbits.CKE = 0; // Data is shifted on falling edge
  ...
  SPI1BRG = 0; //Set baude rate generator to 0 (to maximum disccussed below)
  SPI1CONbits.ENHBUF = 0; // Disable Enhanced Buffer mode
  SPI1CONbits.ON = 1; // Configuration is done, turn on spi

  /*
   * the pbclk2 controls both spi and uart, it is 100 MHz, and the formula is
   * Fsck = pbclk2 / 2 / (BRG + 1), to get the maximum which is 50MHz we set 
   * BRG to 0
   */
}

void spi_send(char code, char *data)
{
  int i;
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    while (!SPI1STATbits.SPIRBF); // receive buffer is empty, loop
...
#define SST26_OP_NOP      0x00  /* No Operation */
#define SST26_OP_WREN     0x06  /* Write Enable */
#define SST26_OP_ULBPR    0x98  /* Global block unlock */
#define SST26_OP_RDID     0x9F  /* Read JEDEC-ID */

int main(void)
{
  char data[5];
  int test_calc = sqrt(2);
